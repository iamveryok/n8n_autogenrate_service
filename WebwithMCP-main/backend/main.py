...
nacos_service = NacosService(service_name=os.getenv("NACOS_SERVICE_NAME"))
service_ip = os.getenv("NACOS_SERVICE_IP")
service_port = os.getenv("BACKEND_PORT")
...
@app.post("/workflow_create")
async def workflow_create(req: ChatRequest = Body(...)):
    # if not chat_db:
    #     raise HTTPException(status_code=503, detail="数据库未初始化")
    ...
    # 复用或生成会话 todo 直接调用脚本历史对话后续进行
    session_id = (req.session_id or str(uuid.uuid4())).strip()

    generator = SimpleAutoGenerator()
    result = generator.auto_generate_and_deploy(content)
    return {
        "success": True,
        "result": result
    }
