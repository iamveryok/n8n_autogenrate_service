...
export function validateWorkflowSpec(input: WorkflowInput): WorkflowSpec {
  ...
  //todo 通过导入标准node节点来提高匹配的准确性 通过调用python程序的方法进行规范化
  ...
  const formattedNodes = (input.nodes || []).map((node, index) => {
    ...
    //todo 添加参考nodes_config.json定义的标准node格式的校验
    // const nodesData = JSON.parse(fs.readFileSync('src/python_src/nodes_config.json', 'utf-8'));
    // const nodeformatter :Map<string, any> = new Map()
    //
