  ...
  private setupToolHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request: any) => {
      ...
      try {
        ...
        // 获取用户输入 作为create_workflow工具的输入
        userInput = args.userInput
        // 保持原有的arg结构不变
        delete args.userInput
          ...
          switch (toolName) {
            ...
            case 'create_workflow':
              try {
                ...
                const createdWorkflow = await this.n8nWrapper.createWorkflow(workflowInput,userInput,args.instance);
  ...
  async run() {
    ...
    try {
      ...
      if (isStandaloneMode) {
        ...
        this.log('info',"n8n_mcp_build start stdio")
