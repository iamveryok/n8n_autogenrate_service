...
export class N8NApiWrapper {
  ...
  // }
    ...
    async createWorkflow(workflowInput: WorkflowInput,userInput:string, instanceSlug?: string): Promise<N8NWorkflowResponse> {
    return this.callWithInstance(instanceSlug, async () => {
      const api = this.envManager.getApiInstance(instanceSlug);
      ...
      try {
        ...
        // 获取 Python 脚本路径
        const scriptPath = path.resolve(__dirname,'..','../src/python_src', 'create_workflow_json.py');
        // 调用 Python 脚本 环境指定后续再试 先写死
        const env = {
            ...process.env,
            PYTHONIOENCODING: 'utf-8',
            PYTHONUTF8: '1'
        };
        const pythonProcess = spawn('D:\\miniconda3\\envs\\n8n_mcp\\python.exe', [scriptPath, userInput], {
            env: env,
            encoding: 'utf-8'  // 指定子进程编码
        });
        let output = '';
        let error = '';
        ...
        const response = await api.post('/workflows',result);
        logger.log(`Workflow created with ID: ${response.data.id}`);
        return response.data;
      } catch (error) {
        return this.handleApiError(`creating workflow ${workflowInput.name}`, error);
  ...
  async activateWorkflow(id: string, instanceSlug?: string): Promise<N8NWorkflowResponse> {
    return this.callWithInstance(instanceSlug, async () => {
      ...
      try {
        ...
        const response = await api.post(`/workflows/${id}/activate`, { active: true });
  ...
  async deactivateWorkflow(id: string, instanceSlug?: string): Promise<N8NWorkflowResponse> {
    return this.callWithInstance(instanceSlug, async () => {
      ...
      try {
        ...
        const response = await api.post(`/workflows/${id}/deactivate`, { active: false });
