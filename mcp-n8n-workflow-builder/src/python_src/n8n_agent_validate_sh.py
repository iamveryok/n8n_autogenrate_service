...
class N8NWorkflowAgent:
    def __init__(self):
        self.llm = BaseChatOpenAI(
            model="deepseek-chat",
            openai_api_key='sk-4e69552cec874f7397224b172583012b',  # 替换为你的API密钥
...
def create_workflow_agent():
    ...
    async def deploy_node(state: AgentState):
        ...
        try:
            save_dir = os.path.join(os.path.dirname(__file__), 'n8n_workflow_json')
...
async def main():
    ...
    logger.info("脚本接受用户输入============",user_input)
    # 初始状态
    ...
    try:
        ...
        logger.info("final_state====>", final_state)
        ...
        if final_state.deployment_result.get('success', False):
            ...
            logger.info("返回的n8n工具执行结果是====>",workflow_json)
            sys.stdout.write(workflow_json)
            sys.stdout.flush()
        else:
            logger.error("❌ 工作流部署失败")
    except Exception as e:
